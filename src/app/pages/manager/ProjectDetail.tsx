...
import { Clock, Image as ImageIcon, History, ChevronDown, ChevronUp, MapPin, CheckCircle, AlertTriangle } from 'lucide-react';
...
export function ManagerProjectDetail() {
  ...
  const progressData = [
    { name: 'Jan', expected: 10, actual: 10 },
    { name: 'Feb', expected: 25, actual: 20 },
    { name: 'Mar', expected: 40, actual: 35 },
    { name: 'Apr', expected: 55, actual: project.percentDone },
  ];

  const allPhotos = project.milestones.flatMap(m => m.history.filter(h => h.thumbnail).map(h => ({
    url: h.thumbnail,
    date: h.date,
    milestone: m.name
  })));
        ...
        <div className="h-48 mt-6">
          ...
          <ResponsiveContainer width="100%" height="100%">
            <AreaChart data={progressData} margin={{ top: 0, right: 0, left: -20, bottom: 0 }}>
              ...
              />
              <Area type="monotone" dataKey="expected" stroke="#9ca3af" fillOpacity={1} fill="url(#colorExpected)" name="Planned %" />
              <Area type="monotone" dataKey="actual" stroke="#2563eb" strokeWidth={2} fillOpacity={1} fill="url(#colorActual)" name="Actual %" />
            </AreaChart>
          </ResponsiveContainer>
        </div>
      </div>
              ...
              >
                <div className="flex-1 pr-4">
                  <div className="flex items-center justify-between mb-2">
                    ...
                    <span className={`font-bold ${milestone.percentDone === 100 ? 'text-green-600' : 'text-blue-600'}`}>
                      {milestone.percentDone}%
                    </span>
                  </div>
                  <div className="w-full bg-gray-100 rounded-full h-1.5">
                    <div 
                      className={`h-1.5 rounded-full transition-all duration-500 ${milestone.percentDone === 100 ? 'bg-green-500' : 'bg-blue-500'}`}
                      style={{ width: `${milestone.percentDone}%` }}
                    />
                  </div>
                  <div className="flex items-center text-xs text-gray-500 mt-2 gap-4">
                    <span>Weight: {milestone.weight}%</span>
                    {milestone.lastUpdate && (
                      <span className="flex items-center gap-1"><Clock className="w-3 h-3" /> Updated {new Date(milestone.lastUpdate).toLocaleDateString()}</span>
                    )}
                  </div>
                </div>
                    ...
                    <div className="p-4">
                      ...
                      {milestone.history.length > 0 ? (
                        <div className="space-y-4">
                          {milestone.history.map((hist, i) => (
                            <div key={i} className="flex gap-3">
                              <div className="w-8 h-8 rounded-full bg-blue-100 text-blue-700 flex items-center justify-center text-xs font-bold shrink-0">
                                {hist.agentName.charAt(0)}
                              </div>
                              <div className="flex-1 bg-white p-3 rounded-lg border border-gray-100">
                                <div className="flex justify-between items-start mb-1">
                                  <span className="text-sm font-medium text-gray-900">{hist.agentName}</span>
                                  <span className="text-xs text-gray-500">{new Date(hist.date).toLocaleDateString()}</span>
                                </div>
                                <p className="text-sm text-gray-600 mb-2">Updated progress to <span className="font-semibold">{hist.percentDone}%</span></p>
                                {hist.note && <p className="text-sm text-gray-500 italic bg-gray-50 p-2 rounded">"{hist.note}"</p>}
                                {hist.thumbnail && (
                                  <div className="mt-2 rounded-lg overflow-hidden border border-gray-200 inline-block">
                                    <img src={hist.thumbnail} alt="Update" className="h-20 w-auto object-cover" />
                                  </div>
                                )}
                              </div>
                            </div>
            ...
            allPhotos.map((photo, idx) => (
              ...
              >
                ...
                <div className="absolute inset-x-0 bottom-0 bg-gradient-to-t from-black/70 to-transparent p-3 pt-8">
                  ...
                  <p className="text-white/80 text-[10px]">{new Date(photo.date).toLocaleDateString()}</p>
                </div>
              </motion.div>
            ))
          ) : (
